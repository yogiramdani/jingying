            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-10">
                        ...
                        <input type="text" class="form-control" name="judul" value="{{ $record->judul  }}" required>
                    </div>
                    <div class="mb-10">
                    ...
                    <div class="mb-10">
                        ...
                        <input type="text" class="form-control" name="gaji" value="{{ $record->gaji  }}" required>
                    </div>
                    <div class="mb-10">
                        <label class="form-label">Deskripsi</label>
                        <textarea class="form-control" name="deskripsi" id="kt_docs_ckeditor_classic"> {{ $record->deskripsi  }}</textarea>
                    </div>
                </div>
            ...
            <div class="card shadow-sm">
                ...
                @if(!empty($category))
                    @foreach($category as $row)
                        @php 
                            $cat = json_decode($record->category);
                            $checked = "";
                            if(!empty($cat)){
                                foreach($cat as $val){
                                    if($val->id == $row->id){
                        ...
                        <div class="form-check form-check-custom form-check-solid mb-2">
                            <input class="form-check-input" type="checkbox" name="category[]" value="{{ $row->id }}" id="flexCheckChecked{{ $row->id }}" {{ $checked }}/>
